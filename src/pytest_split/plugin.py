...
def pytest_configure(config: Config) -> None:
    ...
    Configure plugin.
    """
    # Always enable cache writes
    if (config.option.splits and not config.option.group) or (
        config.option.group and not config.option.split
    ):
        warn(
            'It looks like you passed an argument for pytest-split, '
            'but both `splits` and `group` are required for pytest-split to run'
        )
...
class PytestSplitPlugin:
    ...
    def __init__(self, config: Config) -> None:
        ...
        if not self.cached_durations:
            warn(
                ...
                'so you should expect better results in following runs, when test timings have been documented.'
            )

    def pytest_collection_modifyitems(self, config: Config, items: List[nodes.Item]):
        ...
        if splits and group:
            ...
            start_idx, end_idx = self._calculate_suite_start_and_end_idx(
    ...
    def _calculate_suite_start_and_end_idx(
        splits, group: int, items: List[nodes.Item], stored_durations
    ):
